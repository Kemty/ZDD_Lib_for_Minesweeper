...
class MinsweeperLogs_with_deleting:
    ...
    def record(self, root:Deletable_ZDD_Node_for_Minesweeper) -> None:
        """
        ...
        self.size_ratio[i] = max(self.size_ratio[i], n/(root.zdd.create_cnt-root.zdd.delete_cnt+1e-6)) # 0除算回避用に小さい値を分母に足した
...
class MinesweeperSolver_with_deleting_for_experiment3:
    def __init__(self, minesweeper:Minesweeper):
        ...
        self.logs = MinsweeperLogs_with_deleting(S)
...
if __name__ == "__main__":
    ...
    while win_cnt < target:
        ...
        solver = MinesweeperSolver_with_deleting_for_experiment3(minesweeper)
